...
- Next.js + FastAPI のTodoアプリリポジトリを用意（済）
- GitHub Issuesに4つのタスクを登録（済）
  - Issue #1: 「フロントエンド: 残タスク数の表示とClear Completedボタンを追加」
  - Issue #2: 「API: Todoタイトルの編集エンドポイントを追加」
  - Issue #3: 「バグ: フィルター適用中にTodoを追加すると表示されない」
  - Issue #4: 「調査: page.tsx のコンポーネント分割とカスタムフック抽出の検討」

#### デモ本番

起動スクリプト1発で4つのClaude Codeセッションが立ち上がる。

```bash
./start_demo.sh
```

> tmuxで2x2のペインが開き、各ペインでIssueの内容がClaude Codeに渡される。
> 操作: `Ctrl+B` → 矢印キー でペイン切り替え。

| ペイン | Issue | Claude Codeへの指示 |
|---|---|---|
| 左上 | #1 フロント修正 | 実装 → ブランチ → コミット |
| 右上 | #2 API修正 | 実装 → ブランチ → コミット |
| 左下 | #3 バグ修正 | 修正 → ブランチ → コミット |
| 右下 | #4 リファクタ調査 | 調査 → docs/にドキュメント出力 |
...
# タブB — API側修正（エンドポイント追加）
gh issue view 2 | claude
→ 「このIssueを実装して。ブランチを切ってコミットまでやって」

# タブC — バグ修正
gh issue view 3 | claude
→ 「このIssueを修正して。ブランチを切ってコミットまでやって」

# タブD — リファクタの事前調査（コード変更なし）
gh issue view 4 | claude
→ 「このIssueの調査をして。結果をdocs/にまとめて」
```

4. **4つが同時に進む様子を見せる**
   - タブA/B/C：コードが書かれていく
   - タブD：コードを読んで分析・ドキュメント生成
5. 完了後、各ブランチのPRを作成
